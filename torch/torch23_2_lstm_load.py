...
with torch.no_grad():
    ...
    for x_test, y_test in train_loader:
        h0 = torch.zeros(1, x_test.shape[0], 32).to(DEVICE)  # (num_layers, batch_size, hidden_size) = (5, 1, 64)
        c0 = torch.zeros(1, x_test.shape[0], 32).to(DEVICE)  # (num_layers, batch_size, hidden_size) = (5, 1, 64)
