...
#2. 모델
model = Sequential()
model.add(Conv2D(100, (3,3), activation='relu', input_shape=(32, 32, 3)))   # 데이터의 개수(n장)는 input_shape 에서 생략, 3차원 가로세로컬러  27,27,10
model.add(Conv2D(filters=50, activation='relu', kernel_size=(3,3)))                       
...
# path ='./_save/keras35_07/'
# filename = '{epoch:04d}-{val_loss:.4f}.hdf5'    # 1000-0.7777.hdf5    { } = dictionary, 키와 밸류  d는 정수, f는 소수
# filepath = "".join([path, 'k35_07', date, '_', filename])      # 파일위치와 이름을 에포와 발로스로 나타내준다
...
start = time.time()
hist = model.fit(x_train, y_train, epochs=1000, batch_size=12800,
          verbose=1,
