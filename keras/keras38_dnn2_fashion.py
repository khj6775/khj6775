...
path ='./_save/keras38_dnn2/'
filename = '{epoch:04d}-{val_loss:.4f}.hdf5'    # 1000-0.7777.hdf5    { } = dictionary, 키와 밸류  d는 정수, f는 소수
filepath = "".join([path, 'k38_02', date, '_', filename])      # 파일위치와 이름을 에포와 발로스로 나타내준다
...
start = time.time()
hist = model.fit(x_train, y_train, epochs=1000, batch_size=2058,
          verbose=1,
